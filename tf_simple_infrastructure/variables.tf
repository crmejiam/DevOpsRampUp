...
variable "instance_name" {
  ...
  type        = list(string)
  default     = ["frontend-rampup-cristian.mejiam", "backend-rampup-cristian.mejiam", "dbrampupcmm"]
}

variable "volume_name" {
  description = "List with the names of each volume"
  type        = list(string)
  default     = ["frontVol-rampup-cristian.mejiam", "backVol-rampup-cristian.mejiam"]
}
...
variable "provisioner_file" {
  ...
  type        = list(string)
  default     = ["Provision-frontend.tpl", "Provision-backend.tpl"]
}
...
variable "ssh_port" {
  description = "Port designed for ssh access"
  type        = number
  default     = 22
}
...
variable "privateIP" {
  ...
  default = ["10.1.0.10", "10.1.80.8"]  # 10.1.0.10 for front, 10.1.80.0 for back
}
